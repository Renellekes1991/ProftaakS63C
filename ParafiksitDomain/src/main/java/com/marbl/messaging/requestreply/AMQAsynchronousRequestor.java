 ...
 * @author Maja Pesic
 */
public class AMQAsynchronousRequestor<REQUEST, REPLY> {
    /**
     ...
     * @param senderQueue
     */
    public AMQAsynchronousRequestor(String factoryName, String requestSenderQueue, String replyReceiverQueue, IRequestReplySerializer<REQUEST, REPLY> serializer) throws Exception {
        super();
    ...
    public void start() {
        try
        {
            ...
            Logger.getLogger(AMQAsynchronousRequestor.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public synchronized void sendRequest(REQUEST request, IReplyListener<REQUEST, REPLY> listener) {
        try {
            TextMessage message = gateway.createMessage(serializer.requestToString(request));
